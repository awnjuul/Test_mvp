...
# Простий набір ключових слів для трьох секторів — можна доповнювати
SECTOR_KEYWORDS = {
    'agrarian': [
        'земл', 'сільськогосподар', 'фермер', 'підтримк', 'субсид', 'експорт', 'хліб', 'зерно', 'фітосаніт', 'фітосанитар', 'квот', 'аграр', 'паї'
    ],
    'social': [
        'труд', 'зарплат', 'страхов', 'пенс', 'пенсій', 'праці', 'охорон', 'соціал', 'внеск', 'штраф', 'відпустк'
    ],
    'corporate': [
        'подат', 'валют', 'корпоратив', 'управл', 'm&a', 'злит', 'придб', 'борг', 'вій', 'ліміт', 'концерн'
    ]
}

BASE_PORTAL = 'https://itd.rada.gov.ua'
PERIOD_LIST_URL = 'https://itd.rada.gov.ua/billInfo/Bills/period'  # heuristic
...
def get_soup(html):
    """Пробуємо використати lxml, якщо недоступно — html.parser"""
    try:
...
def fetch_period_page(period_days=30):
    try:
...
def extract_bill_links(html, limit=100):
    soup = get_soup(html)
    links = []
...
def save_rows_to_csv(rows, output=OUTPUT_CSV):
    fieldnames = ['bill_url', 'title', 'published_date', 'status', 'matched_sectors', 'keywords_found', 'risk_score', 'summary']
...
def main(period_days=30, limit=50):
    html = fetch_period_page(period_days)
    if not html:
    ...
    if rows:
        # залишаємо тільки перші 100 записів
        rows = rows[:500]
